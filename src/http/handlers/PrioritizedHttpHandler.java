...
public class PrioritizedHttpHandler extends BaseHttpHandler implements HttpHandler {
    ...
    public PrioritizedHttpHandler(TaskManager tm) {
        super(tm);
    }
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        switch (exchange.getRequestMethod()) {
            case "GET":
                if (getLastPartOfPath(exchange).equals("prioritized") && getPartsOfPathNumber(exchange) == 2) {
                    final String response = gson.toJson(tm.getPrioritizedTasks());
